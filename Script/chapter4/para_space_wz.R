...
#assume r_ins = r_del = r
r       = runif(nd,0.05,0.15)
rt      = r*tv
g.open  = (1-exp(-2*rt))/2  #the probabilities of U or V are both (1 − η)/2.
...
#setup the omegaz proportional to omega
wz = runif(nd,0.02,0.5)

##output the 100 parameters
tPar = matrix(0,100,16)
for (i in 1:nd) {
  tPar[i,1:4]  = Pi.all[,i]
...
colnames(tPar)=c('A','C','G','T','s1','s2','s3','s4','s5','s6','omega','tau','ext.I','ext.D','r*t','omegaZ')

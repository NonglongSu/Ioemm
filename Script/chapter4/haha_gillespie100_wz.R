...
sim_del = function(s, dnaB, pos, posB, k, wz){
  po = pos %% 3
  flag = 0
  if(po == 2){#phase 1
...
sim_ins = function(s, dnaB, pos, posB, k, wz){
...
D_onestep = function(r1,ext,dna0,wz,omegal){
  dnaB = dna0
  tau  = 0
  #total rates
  phase   = (seq_along(dnaB)+2) %% 3 + 1
  phase.r = cbind(r1,rep(0,length(phase)))
  ...
  for (iter in 1:500) {
    ...
    k = rgeom(1,1-ext)
    k = 3*(k+1)
    if((length(dna)-pos)<k){next} #delete more than the end 
    ...
    dnaB = sim_del(dna, dnaB, pos, Pos, k, wz.tmp)
  }
...
I_onestep= function(r1,ext,dnaB1,wz,omegal){
  tau = 0
  ...
  phase.r[1,1] = r1
  phase.r[2:nrow(phase.r),] = cbind(r1,rep(0,length(phase)))
  #no events should occur again on the deletion box
  ...
  for (iter in 1:500) {
    ...
    if(posB == 0){#immortal link
      ...
      Isinsert = 1 #succeed--always syn
    }else{
      ngap     = length(which(phase.r[(1:posB),1]==0))
      #update dna
      dna      = dnaB1[which(dnaB1!='-')]
      pos      = posB-ngap
      dnaBs    = sim_ins(dna, dnaB1, pos, posB, k, wz.tmp)
      dnaB1    = dnaBs[[1]]
      Isinsert = dnaBs[[2]]
    }
    
    if(Isinsert==1){#succeed
      Bi      = append(phase.r[,2],rep(1,k),Pos)     
      Rate    = append(phase.r[,1],rep(r1,k),Pos)
...
#ouD = "k12/Gs/2"
#inF = "trueP.100.txt"
main = function(ouD, inF, l, omega_z, ss, omega_l){
  ...
  omegaz = as.numeric(omega_z)  #znzs: omega_z=0 (default)
  if(omegaz==1){
    wz = 1
  }else{
    wz = tp[16]
  ...
  set.seed(8088)
  len   = as.numeric(l)    #default:999
  dna0  = sample(DNA_BASES, len, prob=Pi, replace=TRUE)
  dna00 = paste0(dna0,collapse='')
  
  sim.IDS = list()
  ssize   = as.numeric(ss) #default:500
  for(i in 1:ssize){
    dnaB  = D_onestep(r1,extend[2],dna0,wz,omegal)
    dnaB1 = S_onestep(r2,gtr,dnaB)
    dnaB2 = I_onestep(r1,extend[1],dnaB1,wz,omegal)
